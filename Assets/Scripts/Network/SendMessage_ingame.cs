...
public class SendMessage_ingame : MonoBehaviourPunCallbacks
{
    ...
    public void ingame_Send()
    {
        ...
        PV.RPC("ingame_ChatRPC", RpcTarget.All, msg, msg2);
    ...
    void ingame_ChatRPC(string msg1, string msg2)
    {
        bool isInput = false;
        ...
        for (int i = 0; i < ChatText.Length; i++)
            ...
            {
                isInput = true;
                ChatText[i].text = "<color=" + myColor[PhotonNetwork.LocalPlayer.ActorNumber -1] + ">" + msg1 + "</color>" + msg2;
                break;
            }
        if (!isInput) // 꽉차면 한칸씩 위로 올림
        {
            for (int i = 1; i < ChatText.Length; i++) ChatText[i - 1].text = ChatText[i].text;
            ChatText[ChatText.Length - 1].text = "<color=" + myColor[PhotonNetwork.LocalPlayer.ActorNumber -1] + ">" + msg1 + "</color>" + msg2;
        }
    }
    #endregion
}
